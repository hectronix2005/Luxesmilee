    ...
    try {
        ...
        if (savedData) {
            try {
                ...
                console.log('Site data loaded from localStorage');
                updateSiteContent();
                return;
            } catch (parseError) {
        ...
        if (sessionData) {
            try {
                ...
                console.log('Site data loaded from sessionStorage');
                updateSiteContent();
                return;
            } catch (parseError) {
        ...
        console.log('Using default content due to error');
    }
}
...
function setupFirebaseListener() {
    if (window.firebaseDB) {
        ...
        window.firebaseDB.onSnapshot(docRef, (doc) => {
            if (doc.exists()) {
                siteData = doc.data();
                updateSiteContent();
                console.log('Site content updated from Firebase in real-time');
            }
        }, (error) => {
...
// Setup Firebase listener after a short delay to ensure Firebase is initialized
setTimeout(() => {
    setupFirebaseListener();
}, 2000);

// Fallback: Auto-refresh content every 10 seconds to catch localStorage changes
setInterval(() => {
